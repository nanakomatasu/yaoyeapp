	...
	<view class="user-spread">
		...
		<view class="header" v-if="!showLoading">
			<view class="user-info row">
				<view class="user-avatar">
					...
					<view class="avatar-tag white" v-if="vipState==2">
						{{localUserLevel == 0 ? '普通用户' : "v" + localUserLevel}}</view>
				</view>
				<view class="user-message-container column ml20 white">
					...
					<view class="xs row" style="line-height: 30rpx;text-align: left;">
						我的邀请人：{{!inviteStatus ? '无' : userInfo.leader.nickname}}
						<view v-if="!inviteStatus" class="br60 white write-btn row-center" @tap="showInvitePop">填写
						</view>
					</view>
				</view>
			</view>
		...
		<view class="main" v-if="!showLoading">
						...
						<view class="vip-form">
							...
							<view @click="showRegion=true">
								<view class="form-item row ">
									...
									<input class="md" name="region" :value="region" disabled type="text"
							...
							<view class="form-item bg-white row" style="height: 74px;padding: 24rpx 20rpx 24rpx 30rpx">
								...
								<u-input type="textarea" v-model="reason" placeholder="（选填）"
									:custom-style="{paddingLeft: '10rpx'}" :fixed="true" style="flex: 1" />
			...
			<view :hidden="vipState!=1" class="user-result-box">
				<view class="user-result column-center">
					...
					<view class="user-result-content column-center">
						<image class="apply-result-img"
							:src="applyObject.status == 2 ? '/static/images/icon_payFail.png' : '/static/images/icon_paySuccess.png'" />
						<view class="mt10 nr" style="line-height: 40rpx">{{applyObject.status_str}}</view>
						<view class="apply-fail-reason sm"
							:style="applyObject == 2 ? 'visibility: none' : 'visibility: hidden'">{{applyObject.reason}}
						</view>
					</view>
				...
				<view
					:class="'white mt20 br60 apply-btn row-center md ' + (applyObject.status == 0 ? 'bg-gray' : 'bg-primary')"
					@tap="reApply">重新申请</view>
				<view class="mt20 xxs lighter row-center">提交成功后，我们将会在1-2个工作日内给您回复</view>
			</view>
			<!-- 已申请 -->
			<view :hidden="vipState!=2" class="user-vip">
				<view class="user-assets-box">
					<view class="user-assets-header row-between">
						...
						<navigator hover-class="none" class="primary-btn white row-center"
							url="/pages/bundle/user_withdraw/user_withdraw">提数权</navigator>
					</view>
				</view>
				<view class="mt20 fans-msg-box row bg-white md">
					...
					<navigator url="/pages/bundle/invite_fans/invite_fans" hover-class="none"
						class="invite-fans row-center normal">
						邀请好友
						<u-icon class="ml10" name="arrow-right" size="28rpx" color="#666" />
					</navigator>
				</view>
				<view class="my-invite-box mt20 bg-white column-center">
					...
					<view class="row bold mt20" style="font-size: 42rpx;line-height: 30rpx">
						...
						<view class="invite-copy-btn ml10 xxs" @click="onCopy">点击复制</view>
					</view>
				...
				<view class="usual-tools-box bg-white mt20">
					...
					<view class="usual-content row">
						<navigator hover-class="none" url="/pages/bundle/invite_fans/invite_fans"
							class="usual-item column-center">
...
<script>
	...
	export default {
		...
		onShow() {
			...
			this.localUserLevel = user.level
