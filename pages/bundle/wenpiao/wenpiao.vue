	...
	<view class="wenpiao-container">
		...
		<view class="exchange-section">
			...
			<view class="hint-section">
				<text class="hint">可用兑票:{{duipiao}}</text>
			...
			<text class="hint">可用补贴:{{butie}}</text>
		</view>
	...
	export default {
		data() {
			return {
				...
				duipiao: '',
				butie: '',
				exchangeType: 'ticket', // 'ticket' or 'mixed'
			}
		},
		computed: {
			currentTimeLabels() {
		...
		methods: {
			...
			confirmExchange() {
				...
				uni.showActionSheet({
					itemList: ['兑票积分兑换', '50%兑票积分+50%补贴积分兑换'],
					success: (res) => {
						const exchangeType = res.tapIndex === 0 ? 1 : 2;
						wenpiaoExchange({
			...
			getZhexian() {
				zhexian({
					...
					if (res.code === 1) {
						...
						this.duipiao = user.ticket_points
						this.butie = user.subsidy_points
						const chartsData = []
						this.hourTimeLabels = []
			...
			toExchangeRecord() {
				uni.navigateTo({
					url:'/pages/bundle/user_bill/user_bill?type=0&score=8'
				})
			},
			switchTimeType(type) {
