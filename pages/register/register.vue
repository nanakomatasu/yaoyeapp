		...
		<view class="input-container">
			...
			<view class="input-item row" v-if="appConfig.register_setting">
				...
				<button class="bd-primary sm primary br60 row-center" @click="sendSmsFun()">
					...
					<u-count-down ref="countDown" :show-days="false" :timestamp="time" separator="zh" color="#FF2C3C"
						separatorColor="#FF2C3C" bg-color="rgba(0, 0, 0, 0)" :show-hours="false" :show-minutes="false"
						:autoplay="false" v-show="!canSendSms" @end="countDownFinish()" />
				</button>
			</view>
			<view class="input-item row">
			...
			<view class="input-item row">
				<view class="input-label row md normal">邀请码</view>
				<input type="text" v-model="invite" placeholder="请输入邀请码" :disabled="disabledStatus" />
			</view>
		</view>
		<checkbox :checked="isAgree" icon-size="15px" :checked-color="primaryColor" @click="changeChecked">
			<view class="sm row">
				已阅读并同意
				<navigator @tap.stop="" class="primary" hover-class="none"
...
<script>
	...
	import { downLoadUrl } from '@/config/app.js'
