<template>
	<view class="float-tab ~column">
		<navigator class="tab-img" :style="{top: (showCart ? 3 : 2)*top + 'px'}" hover-class="none" open-type="switchTab"
			url="/pages/index/index">
			<image class="tab-icon" src="/static/images/icon_float_home.png" />
		</navigator>
		<navigator class="tab-img" :style="{top: (showCart ? 2 : 1)*top + 'px'}" hover-class="none" open-type="navigate"
		...
		<navigator class="tab-img" :style="{top: top + 'px'}" hover-class="none" open-type="switchTab"
			url="/pages/shop_cart/shop_cart" v-if="showCart">
